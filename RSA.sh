...
for size in ${key_sizes}; do
	...
	openssl genrsa -des3 -out ${dir}private${size} -passout pass:${pass}12345 ${size}

	echo "Extracting public key(${size}) in ${dir}"
    openssl rsa -in ${dir}private${size} -outform PEM -pubout -out ${dir}public${size} -passin pass:${pass}12345
done
